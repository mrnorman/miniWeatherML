...
namespace custom_modules {
  ...
  class Dynamics_Euler_Stratified_WenoFV {
    ...
    void time_step(core::Coupler &coupler, real &dt_phys, MODEL &model) {
      ...
      for (int icycle = 0; icycle < ncycles; icycle++) {
        ...
        // Stage 1
        //////////////
        compute_tendencies( coupler , state     , state_tend , tracers     , tracers_tend , dt_dyn , model );
        // Apply tendencies
        ...
        // Stage 2
        //////////////
        compute_tendencies( coupler , state_tmp , state_tend , tracers_tmp , tracers_tend , (1._fp/4._fp) * dt_dyn , model );
        // Apply tendencies
        parallel_for( YAKL_AUTO_LABEL() , Bounds<4>(nz,ny,nx,nens) , YAKL_LAMBDA (int k, int j, int i, int iens) {
          for (int l = 0; l < num_state  ; l++) {
        ...
        // Stage 3
        //////////////
        compute_tendencies( coupler , state_tmp , state_tend , tracers_tmp , tracers_tend , (2._fp/3._fp) * dt_dyn , model );
        // Apply tendencies
        parallel_for( YAKL_AUTO_LABEL() , Bounds<4>(nz,ny,nx,nens) , YAKL_LAMBDA (int k, int j, int i, int iens) {
          for (int l = 0; l < num_state  ; l++) {
    ...
    void compute_tendencies( core::Coupler &coupler , real5d const &state   , real5d const &state_tend   ,
                                                      real5d const &tracers , real5d const &tracers_tend , real dt ,
                                                      MODEL &model ) const {
      using yakl::c::parallel_for;
      using yakl::c::Bounds;
      using std::min;
