...
namespace modules {
  ...
  class Dynamics_Euler_Stratified_WenoFV {
    ...
    int  static constexpr idU = 0;  // u-momentum
    int  static constexpr idV = 1;  // v-momentum
    int  static constexpr idW = 2;  // w-momentum
    int  static constexpr idT = 3;  // Density * potential temperature
    ...
    real2d      ref_density_cells;
    real2d      ref_theta_cells;
    real2d      ref_pressure_cells;
    ...
    void convert_dynamics_to_coupler( core::Coupler &coupler , realConst5d state , realConst5d tracers ) const {
      ...
      parallel_for( YAKL_AUTO_LABEL() , Bounds<4>(nz,ny,nx,nens) , YAKL_LAMBDA (int k, int j, int i, int iens) {
        ...
        real u     = state(idU,hs+k,hs+j,hs+i,iens) / rho_r;
        real v     = state(idV,hs+k,hs+j,hs+i,iens) / rho_r;
        real w     = state(idW,hs+k,hs+j,hs+i,iens) / rho_r;
        real theta = state(idT,hs+k,hs+j,hs+i,iens) / rho_r;
        real rho_v = tracers(idWV,hs+k,hs+j,hs+i,iens);
        real p_r   = ref_pressure_cells(k,iens);

        real R = (rho_d*R_d + rho_v*R_v) / (rho_d + rho_v);
        dm_temp(k,j,i,iens) = theta*R_d/R*std::pow( p_r/p0 , R_d/cp_d );
    ...
    void convert_coupler_to_dynamics( core::Coupler const &coupler , real5d &state , real5d &tracers ) const {
      ...
      // Compute reference pressure as average pressure column from coupler data
      parallel_for( YAKL_AUTO_LABEL() , Bounds<4>(nz,ny,nx,nens) , YAKL_LAMBDA (int k, int j, int i, int iens) {
      ...
      parallel_for( YAKL_AUTO_LABEL() , Bounds<4>(nz,ny,nx,nens) , YAKL_LAMBDA (int k, int j, int i, int iens) {
        ...
        real rho_v = dm_tracers(idWV,k,j,i,iens);
        state(idU,hs+k,hs+j,hs+i,iens) = rho_r*u;
        state(idV,hs+k,hs+j,hs+i,iens) = rho_r*v;
        state(idW,hs+k,hs+j,hs+i,iens) = rho_r*w;
        state(idT,hs+k,hs+j,hs+i,iens) *= rho_r;
