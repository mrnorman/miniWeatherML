...
namespace modules {
  ...
  class Dynamics_Euler_Stratified_WenoFV {
    ...
    void compute_tendencies( core::Coupler &coupler , real5d const &state   , real5d const &state_tend   ,
      ...
      real1d beta_acoust("beta_acoust",nens);
      real1d beta_advect("beta_advect",nens);
      if (coupler.get_data_manager_readonly().entry_exists("trainable_parameters")) {
        auto params = coupler.get_data_manager_readonly().get<real const,2>("trainable_parameters");
        params.slice<1>(0,0).deep_copy_to(beta_acoust);
      ...
      parallel_for( YAKL_AUTO_LABEL() , Bounds<4>(nz+1,ny+1,nx+1,nens) , YAKL_LAMBDA (int k, int j, int i, int iens) {
        ...
        if (j < ny && k < nz) {
          ...
          real ru = 0.5_fp*(ru_R+ru_L - beta_acoust(iens)*mx/cs2*(p_R -p_L ));
          real p  = 0.5_fp*(p_R +p_L  - beta_acoust(iens)*mx    *(ru_R-ru_L));
          // Advective
          ...
          state_flux_x(idU,k,j,i,iens) = 0.5_fp*(ru*(u_R+u_L) - beta_advect(iens)*mx*(u_R-u_L)) + p;
          state_flux_x(idV,k,j,i,iens) = 0.5_fp*(ru*(v_R+v_L) - beta_advect(iens)*mx*(v_R-v_L));
          state_flux_x(idW,k,j,i,iens) = 0.5_fp*(ru*(w_R+w_L) - beta_advect(iens)*mx*(w_R-w_L));
          state_flux_x(idT,k,j,i,iens) = 0.5_fp*(ru*(t_R+t_L) - beta_advect(iens)*mx*(t_R-t_L));
          for (int tr=0; tr < num_tracers; tr++) {
            real tr_L = tracers_limits_x(tr,0,k,j,i,iens)/r_L;
            real tr_R = tracers_limits_x(tr,1,k,j,i,iens)/r_R;    
            tracers_flux_x(tr,k,j,i,iens) = 0.5_fp*(ru*(tr_R+tr_L) - beta_advect(iens)*mx*(tr_R-tr_L));
          }
        }
        ...
        if ( (! sim2d) && i < nx && k < nz) {
          ...
          real p_L  = C0 * std::pow(rt_L,gamma);           real p_R  = C0 * std::pow(rt_R,gamma);
          // Tunable Lax-Friedrichs
          real rv = 0.5_fp*(rv_R+rv_L - beta_acoust(iens)*mx/cs2*(p_R -p_L ));
          real p  = 0.5_fp*(p_R +p_L  - beta_acoust(iens)*mx    *(rv_R-rv_L));
          // Advective
          ...
          state_flux_y(idU,k,j,i,iens) = 0.5_fp*(rv*(u_R+u_L) - beta_advect(iens)*mx*(u_R-u_L));
          state_flux_y(idV,k,j,i,iens) = 0.5_fp*(rv*(v_R+v_L) - beta_advect(iens)*mx*(v_R-v_L)) + p;
          state_flux_y(idW,k,j,i,iens) = 0.5_fp*(rv*(w_R+w_L) - beta_advect(iens)*mx*(w_R-w_L));
          state_flux_y(idT,k,j,i,iens) = 0.5_fp*(rv*(t_R+t_L) - beta_advect(iens)*mx*(t_R-t_L));
          for (int tr=0; tr < num_tracers; tr++) {
            real tr_L = tracers_limits_y(tr,0,k,j,i,iens)/r_L;
            real tr_R = tracers_limits_y(tr,1,k,j,i,iens)/r_R;    
            tracers_flux_y(tr,k,j,i,iens) = 0.5_fp*(rv*(tr_R+tr_L) - beta_advect(iens)*mx*(tr_R-tr_L));
          }
        } else if (i < nx && k < nz) {
        ...
        if (i < nx && j < ny) {
          ...
          real p_L  = C0 * std::pow(rt_L,gamma);           real p_R  = C0 * std::pow(rt_R,gamma);
          // Tunable Lax-Friedrichs
          real rw = 0.5_fp*(rw_R+rw_L - beta_acoust(iens)*mx/cs2*(p_R -p_L ));
          real p  = 0.5_fp*(p_R +p_L  - beta_acoust(iens)*mx    *(rw_R-rw_L));
          // Advective
          ...
          state_flux_z(idU,k,j,i,iens) = 0.5_fp*(rw*(u_R+u_L) - beta_advect(iens)*mx*(u_R-u_L));
          state_flux_z(idV,k,j,i,iens) = 0.5_fp*(rw*(v_R+v_L) - beta_advect(iens)*mx*(v_R-v_L));
          state_flux_z(idW,k,j,i,iens) = 0.5_fp*(rw*(w_R+w_L) - beta_advect(iens)*mx*(w_R-w_L)) + p;
          state_flux_z(idT,k,j,i,iens) = 0.5_fp*(rw*(t_R+t_L) - beta_advect(iens)*mx*(t_R-t_L));
          for (int tr=0; tr < num_tracers; tr++) {
            real tr_L = tracers_limits_z(tr,0,k,j,i,iens)/r_L;
            real tr_R = tracers_limits_z(tr,1,k,j,i,iens)/r_R;    
            tracers_flux_z(tr,k,j,i,iens) = 0.5_fp*(rw*(tr_R+tr_L) - beta_advect(iens)*mx*(tr_R-tr_L));
          }
        }
