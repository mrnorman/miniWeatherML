...
namespace modules {
  ...
  inline void perturb_temperature( core::Coupler &coupler , bool do_random = false , bool do_bubble = true ) {
    ...
    if (do_random) {
      ...
      int ncol = ny*nx*nens;
      size_t seed = static_cast<size_t>(coupler.get_myrank()*nz*ncol);

      parallel_for( YAKL_AUTO_LABEL() , SimpleBounds<2>(num_levels,ncol) , YAKL_LAMBDA (int k, int i) {
    ...
    if (do_bubble) {
      ...
      parallel_for( YAKL_AUTO_LABEL() , SimpleBounds<4>(nz,ny,nx,nens) , YAKL_LAMBDA (int k, int j, int i, int iens) {
        ...
        if (rad < 1) temp(k,j,i,iens) += amp * pow( cos(M_PI*rad/2) , 2._fp );
      });
    }
  }
