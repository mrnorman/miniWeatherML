...
namespace modules {
  ...
  inline void sponge_layer( core::Coupler &coupler , real dt ) {
    ...
    havg_fields = 0;
    parallel_for( YAKL_AUTO_LABEL() , SimpleBounds<5>(num_fields,num_layers,ny,nx,nens) ,
    ...
    parallel_for( YAKL_AUTO_LABEL() , SimpleBounds<5>(num_fields,num_layers,ny,nx,nens) ,
      ...
      full_fields(ifld,k,j,i,iens) += ( havg_fields(ifld,kloc,iens)/(nx_glob*ny_glob*nranks) -
                                        full_fields(ifld,k,j,i,iens) ) * factor;
    });
  }
