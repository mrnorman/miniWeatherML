...
SUBROUTINE KESSLER(theta, qv, qc, qr, rho, pk, dt, z, nz, precl)  bind(C,name="kessler_fortran")
  ...
  real(c_double), DIMENSION(nz), INTENT(INOUT) :: &
            theta   ,     & ! Potential temperature (K)
            qv      ,     & ! Water vapor mixing ratio (gm/gm)
            qc      ,     & ! Cloud water mixing ratio (gm/gm)
            qr              ! Rain  water mixing ratio (gm/gm)

  real(c_double), DIMENSION(nz), INTENT(IN) :: &
            rho             ! Dry air density (not mean state as in KW) (kg/m^3)
  ...
  integer(c_int), INTENT(IN) :: nz ! Number of thermodynamic levels in the column
  ...
  do nt=1,rainsplit
    ...
    do k=1,nz
      ...
      qrprod = qc(k) - (qc(k)-dt0*max(.001d0*(qc(k)-.001d0),0.))/(1.d0+dt0*2.2d0*qr(k)**.875d0)
      ...
      ern = min(dt0*(((1.6d0+124.9d0*(r(k)*qr(k))**.2046d0)  &
